...
class CarlaCore:
    ...
    def spawn_vehicle(self):
        ...
        try:
            ...
            while self.vehicle is None:
                ...
                # self.vehicle = self.world.try_spawn_actor(self.bp, self.spawn_points[0])
            ...
            logger.debug('Creating Ground Truth Manager...')

            self.gt_manager = GTManager(self)
    ...
    def move_vehicle(self):
        ...
        try:
            ...
            # self.vehicle.set_transform(self.spawn_points[0])
    ...
    def create_hazards(self):
        '''
        Create hazards (accidents and construction areas) in the environment.
        '''
        bp_lib = self.world.get_blueprint_library()

        # Get the blueprints related to hazard environments.
        iron_plank = bp_lib.find('static.prop.ironplank')
        barrel = bp_lib.find('static.prop.barrel')
        street_barrier = bp_lib.find('static.prop.streetbarrier')
        traffic_cone = bp_lib.find('static.prop.constructioncone')
        construction_cone = bp_lib.find('static.prop.trafficcone01')
        construction_warning = bp_lib.find('static.prop.warningconstruction')
        accident_warning = bp_lib.find('static.prop.warningaccident')
        traffic_warning = bp_lib.find('static.prop.trafficwarning')

        humans = bp_lib.filter('walker.pedestrian.*')

        # hazard_point = self.map.get_waypoint(self.spawn_points[1].location)
        hazard_point = self.map.get_waypoint(random.choice(self.spawn_points_backup).location)

        road_length_front = 0.1 * (len(hazard_point.next_until_lane_end(0.1)) - 1)
        road_length_back = hazard_point.s

        max_hazard_length = min(road_length_front, road_length_back)

        if max_hazard_length > 2.0:
            hazard_length = random.uniform(2.0, max_hazard_length)

            front_waypoints = hazard_point.next_until_lane_end(2.0)
            back_waypoints = hazard_point.previous_until_lane_start(2.0)

            num_waypoints = min(min(len(front_waypoints), len(back_waypoints)) - 1, int(np.floor(hazard_length / 2.0)))

            element = random.choice(['barrel', 'ironplank', 'human', 'none'])
            self.spawn_construction_element(hazard_point, bp_lib, element)

            for wp in front_waypoints[:num_waypoints]:
                element = random.choice(['barrel', 'ironplank', 'human', 'none'])
                self.spawn_construction_element(wp, bp_lib, element)

            for wp in back_waypoints[:num_waypoints]:
                element = random.choice(['barrel', 'ironplank', 'human', 'none'])
                self.spawn_construction_element(wp, bp_lib, element)

            self.spawn_construction_element(back_waypoints[num_waypoints], bp_lib, 'street_barrier')
            self.spawn_construction_element(front_waypoints[num_waypoints], bp_lib, 'street_barrier')

    def spawn_construction_element(self, wp, bp_lib, element):
        wp_transform = wp.transform

        wp_transform.location.x += random.uniform(-0.32, 0.32)
        wp_transform.location.y += random.uniform(-0.32, 0.32)

        if element == 'street_barrier':
            wp_transform.rotation.yaw += 90.0 + random.uniform(-30.0, 30.0)
        else:
            wp_transform.rotation.yaw += random.uniform(-90.0, 90.0)

        if element == 'barrel':
            self.world.try_spawn_actor(bp_lib.find('static.prop.barrel'), wp_transform)
        elif element == 'ironplank':
            self.world.try_spawn_actor(bp_lib.find('static.prop.ironplank'), wp_transform)
        elif element == 'street_barrier':
            barrier = self.world.try_spawn_actor(bp_lib.find('static.prop.streetbarrier'), wp_transform)
            if barrier is not None:
                barrier.set_collisions(True)
                barrier.set_simulate_physics(True)
        elif element == 'human':
            humans = bp_lib.filter('walker.pedestrian.*')
            ...
            for human in humans:
                if not any(x in human.id for x in ['09', '10', '11', '12', '13', '14', '48', '49']):
                    adults.append(human)

            self.world.try_spawn_actor(random.choice(adults), wp_transform)

        if element != 'street_barrier':
            construction_cone = bp_lib.find('static.prop.trafficcone01')

            wp_right_transform = wp.transform
            wp_left_transform = wp.transform
            
            wp_right_transform.rotation.yaw += 90
            wp_left_transform.rotation.yaw += 90

            wp_right_transform.location += 0.24 * wp.lane_width * wp_right_transform.get_forward_vector()
            wp_left_transform.location -= 0.24 * wp.lane_width * wp_left_transform.get_forward_vector()

            wp_right_transform.location.x += random.uniform(-0.1, 0.1)
            wp_left_transform.location.x += random.uniform(-0.1, 0.1)

            wp_right_transform.location.y += random.uniform(-0.1, 0.1)
            wp_left_transform.location.y += random.uniform(-0.1, 0.1)

            wp_right_transform.rotation.yaw += random.uniform(-90.0, 90.0)
            wp_left_transform.rotation.yaw += random.uniform(-90.0, 90.0)

            self.world.try_spawn_actor(construction_cone, wp_right_transform)
            self.world.try_spawn_actor(construction_cone, wp_left_transform)
    ...
    def tick(self, path=None, scene=None, frame=None, render=False, save=False):
        ...
        if render or save:
            ...
            if self.map_name not in ['Town04', 'Town05', 'Town12', 'Town13']:
                ...
                if torch.min(dists) > 48.0:
                    ...
                    self.gt_manager.get_bounding_boxes()
                    self.gt_manager.get_bev_gt_alt()
        ...
        if save and all(v is not None for v in [path, scene, frame]):
            ...
            self.gt_manager.get_hd_map_info()
