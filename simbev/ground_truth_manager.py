...
class GTManager:
    ...
    def augment_waypoints(self, waypoints: list[carla.Waypoint], scene_duration: int):
        '''
        ...
        self._mapping_area_radius = self._config['mapping_area_radius']
    ...
    def get_area_crosswalks(self, crosswalks: list[carla.Location]):
        ...
        if self._map_name in ['Town01', 'Town02','Town12', 'Town13']:
            for i in range(len(crosswalks)):
                for j in range(i + 1, len(crosswalks)):
                    if crosswalks[i].distance(crosswalks[j]) < 0.02 and \
                        crosswalks[i].distance(self._vehicle.get_location()) < self._mapping_area_radius:
            ...
            for crosswalk in crosswalks:
                ...
                if midpoint.distance(self._vehicle.get_location()) < self._mapping_area_radius:
                    bbox = carla_vector_to_numpy([bbox[0], bbox[2], bbox[6], bbox[4], bbox[0]])
