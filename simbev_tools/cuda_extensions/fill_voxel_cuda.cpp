    ...
    int chunk_size_z = 80)
{
    ...
    TORCH_CHECK(ground_labels.is_cuda(), "ground_labels must be a CUDA tensor.");
    ...
    TORCH_CHECK(ground_labels.dim() == 1 && ground_labels.size(0) == 5, "ground_labels must have shape (5,).");
    ...
    TORCH_CHECK(ground_labels.is_contiguous(), "ground_labels must be contiguous.");
    ...
    TORCH_CHECK(ground_labels.dtype() == torch::kInt32, "ground_labels must be int32.");
