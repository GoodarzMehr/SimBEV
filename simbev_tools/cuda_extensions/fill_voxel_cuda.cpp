...
// C++ interface for hollow voxel filling using ray casting with chunked processing.
    ...
    int chunk_size_z = 80)
{
    ...
    TORCH_CHECK(voxel_grid.dim() == 3, "voxel_grid must be 3D (dim_x, dim_y, dim_z).");
    TORCH_CHECK(priority_map.dim() == 1 && priority_map.size(0) == 32,
                "priority_map must have shape (32,).");
    ...
    TORCH_CHECK(chunk_size_x > 0 && chunk_size_y > 0 && chunk_size_z > 0,
                "chunk sizes must be positive.");
    
    return fill_hollow_voxels_cuda(
        voxel_grid, priority_map, ground_labels, target_class,
        chunk_size_x, chunk_size_y, chunk_size_z);
}

// C++ interface for 3D morphological closing.
torch::Tensor morphological_close_3d(
    torch::Tensor voxel_grid,
    int target_class,
    int kernel_size)
{
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("fill_hollow_voxels_cuda", &fill_hollow_voxels,
          "Fill hollow voxel interiors using 6-direction ray casting with chunked processing (CUDA).",
          py::arg("voxel_grid"),
          py::arg("priority_map"),
          py::arg("ground_labels"),
          py::arg("target_class"),
          py::arg("chunk_size_x") = 200,
          py::arg("chunk_size_y") = 200,
          py::arg("chunk_size_z") = 80);
    m.def("morphological_close_3d_cuda", &morphological_close_3d,
          "Apply 3D morphological closing to a specific class (CUDA).");
}
